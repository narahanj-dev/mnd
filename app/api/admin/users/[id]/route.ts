...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!parsed.success) {
      return Response.json({ error: "수정 내용을 확인하세요." }, { status: 400 });
    }
    ...
    const { error: legacyRequestError } = await admin
      .from("signup_requests")
      .update({ requested_login_id: nextLoginId, department: nextDepartment, approved_user_id: id })
...
async function clearRelatedUserData(admin: AdminClient, target: DeletionTarget) {
  ...
  const nullifyOperations = await Promise.all([
    admin.from("signup_requests").update({ processed_by: null }).eq("processed_by", target.id),
    admin.from("calendar_events").update({ approved_by: null }).eq("approved_by", target.id),
    admin
  ...
  if (ownedEventIds.length > 0) {
    ...
    if (relatedMessageError) return `일정 관련 쪽지 삭제 실패: ${relatedMessageError.message}`;

    const { error: relatedRequestError } = await admin
      .from("event_change_requests")
      .delete()
      .in("event_id", ownedEventIds);
    if (relatedRequestError) return `일정 변경 요청 삭제 실패: ${relatedRequestError.message}`;
  }
  ...
  if (requesterError) return `사용자 변경 요청 삭제 실패: ${requesterError.message}`;
  ...
  if (legacySignupError) return `회원가입 기록 삭제 실패: ${legacySignupError.message}`;
  ...
  const { error: ownedEventError } = await admin
    ...
    .delete()
    .eq("user_id", target.id);
  if (ownedEventError) return `사용자 일정 삭제 실패: ${ownedEventError.message}`;
