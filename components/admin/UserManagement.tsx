        ...
        <>
          ...
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">부서 내에서 이름 또는 아이디로 검색할 수 있습니다. 삭제하면 관련 기록도 모두 삭제됩니다.</p>
            </div>
            {isFullAdmin && <button className="btn-primary" onClick={() => setCreateOpen(true)}>+ 사용자 생성</button>}
          </div>
          {message && <p className="mb-4 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold">{message}</p>}
          <div className="card mb-4 p-4">
            <label className="relative block">
              ...
              <input className="input pl-10" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="이름 또는 아이디로 검색" />
            </label>
          </div>
          ...
          {filteredUsers.length === 0 ? (
            ...
            <div className="card overflow-x-auto">
              <table className="w-full min-w-[1050px] text-sm">
                <thead className="bg-slate-50 text-left"><tr>{["이름", "아이디", "부서", "권한", "상태", "최근 로그인", "관리"].map((heading) => <th key={heading} className="p-3">{heading}</th>)}</tr></thead>
                    ...
                    return (
                      <tr key={user.id}>
                        ...
                        <td className="p-3"><div className="flex flex-wrap gap-2">
                          <button className="btn-secondary text-xs disabled:cursor-not-allowed disabled:opacity-40" disabled={busy || !manageable} title={disabledTitle} onClick={() => setEditUser(user)}>아이디·부서·권한 변경</button>
      ...
      {editUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4" onMouseDown={(event) => event.target === event.currentTarget && setEditUser(null)}>
          <form onSubmit={updateIdentity} className="card w-full max-w-lg space-y-5 p-6">
            <div><h2 className="text-xl font-black">아이디·부서·권한 변경</h2><p className="mt-1 text-sm text-slate-500">{editUser.display_name}님의 정보가 모든 관련 화면에 함께 반영됩니다.</p></div>
            <label className="block text-sm font-bold">로그인 아이디<input name="loginId" className="input mt-1" defaultValue={editUser.login_id} pattern="[A-Za-z0-9_-]{4,30}" required /></label>
      ...
      {deleteUser && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/60 p-4" onMouseDown={(event) => event.target === event.currentTarget && closeDeleteModal()}>
          <form onSubmit={deleteAccount} className="card w-full max-w-lg space-y-5 p-6">
            ...
            <label className="block text-sm font-bold">확인을 위해 아이디 <strong className="text-rose-700">{deleteUser.login_id}</strong> 입력<input value={deleteLoginId} onChange={(event) => { setDeleteLoginId(event.target.value); setDeleteError(""); }} className="input mt-2" placeholder={deleteUser.login_id} autoComplete="off" autoFocus required /></label>
