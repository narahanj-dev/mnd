...
export function MyEventsList() {
  ...
  const [viewerRole, setViewerRole] = useState<UserRole>("department_admin");
  ...
  const load = useCallback(async (department: string | null = null) => {
    ...
    try {
      ...
      const data = await parseJsonResponse<UsersResponse>(
        await fetch(`/api/my-events/users${query}`, { cache: "no-store" }),
      );
      setUsers(data.users);
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((user) =>
      [user.display_name, user.login_id, user.department, USER_ROLE_LABELS[user.role]].some((value) =>
  ...
  if (loading) {
    return <div className="card p-8 text-center text-slate-500">내 일정 사용자 목록을 불러오는 중...</div>;
  }
  ...
  if (!selectedDepartment) {
    return (
      <div>
        <h1 className="mb-2 text-2xl font-black">내 일정</h1>
        <p className="mb-5 text-sm text-slate-500">
          {viewerRole === "admin"
            ? "5개 부서 중 확인할 부서를 선택하세요."
            : `${viewerDepartment} 소속 사용자 일정만 확인할 수 있습니다.`}
        </p>
  ...
  return (
    <div>
      ...
      <div className="mb-5">
        <h1 className="text-2xl font-black">{selectedDepartment} 내 일정</h1>
        <p className="mt-1 text-sm text-slate-500">
          사용자를 검색한 뒤 선택하면 연·월별 신청 내역을 확인할 수 있습니다.
        </p>
      </div>
      ...
      {filteredUsers.length === 0 ? (
        <div className="card p-8 text-center text-slate-500">
          {search ? "검색 결과가 없습니다." : `${selectedDepartment} 소속 사용자가 없습니다.`}
        </div>
      ) : (
