...
export function Header({ profile, unreadCount = 0, pendingCount = 0 }: { profile: Profile; unreadCount?: number; pendingCount?: number }) {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex max-w-[1500px] items-center gap-4 px-4 py-3">
        <Link href="/calendar" className="shrink-0 font-black text-slate-900">부서 공동 연차달력</Link>
        <nav className="flex flex-1 gap-1 overflow-x-auto" aria-label="주요 메뉴">
          {links.map(({ href, label, icon: Icon }) => {
            const active = pathname.startsWith(href);
            const count = href === "/messages" ? unreadCount : href === "/admin/approvals" ? pendingCount : 0;
            return (
              <Link key={href} href={href} className={`flex shrink-0 items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-bold ${active ? "bg-blue-100 text-blue-800" : "text-slate-600 hover:bg-slate-100"}`}>
                <Icon size={16} /> {label}
                {count > 0 && <span className="rounded-full bg-rose-600 px-1.5 py-0.5 text-[10px] text-white">{count}</span>}
              </Link>
            );
          })}
        </nav>
        ...
        <button onClick={logout} disabled={loading} className="btn-secondary flex items-center gap-1.5 text-sm" aria-label="로그아웃">
