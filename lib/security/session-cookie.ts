...
function secret() {
  ...
  if (!value) throw new SecurityError("SESSION_CONFIG", 503, "서버 세션 보안 설정을 확인하세요.");
  return value;
}
...
export function decodeAppSession(value: string): AppSessionPayload | null {
  ...
  try {
    ...
    if (!payload.userId || !Number.isInteger(payload.sessionVersion) || !Number.isFinite(payload.touchedAt)) return null;
    return payload;
  } catch {
    return null;
  }
...
export function isAppSessionExpired(payload: AppSessionPayload) {
  return Date.now() - payload.touchedAt > SESSION_IDLE_SECONDS * 1000;
}
